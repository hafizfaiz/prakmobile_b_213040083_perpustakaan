...
class BookViewModel @Inject constructor(private val bookRepository: BookRepository) : LiveCoroutinesViewModel() {
    ...
    suspend fun insert(
        ...
        _isLoading.postValue(true)
        bookRepository.insert(Book(id, title, author, editionReleaseTimeYear, stock, createdAt, updatedAt, bookRequestId),
            onSuccess = {
                _isLoading.postValue(false)
    ...
    suspend fun update(
        ...
        _isLoading.postValue(true)
        bookRepository.update(Book(id, title, author, editionReleaseTimeYear, stock, createdAt, updatedAt, bookRequestId),
            onSuccess = {
                _isLoading.postValue(false)
                _isDone.postValue(true)
                _book.postValue(true)
